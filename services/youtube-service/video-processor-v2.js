...
class VideoProcessorV2 {
  ...
  async getVideoData(videoId) {
    try {
      ...
      // Solution based on yt-dlp GitHub issue #13211:
      // Use --skip-download with --extractor-args to fix "Did not get any data blocks" error
      let ytdlpOutput = ''
      try {
        ytdlpOutput = execSync(
          `cd "${tempDir}" && yt-dlp --write-auto-subs --sub-lang en --sub-format json3 --skip-download --extractor-args "youtube:player_client=default,-web" --socket-timeout 30 --proxy "${PROXY_URL}" --no-check-certificates -o "${videoId}" "https://www.youtube.com/watch?v=${videoId}" 2>&1`,
          { encoding: 'utf-8', timeout: 300000 }
        )
        console.log(`   📋 Subtitle download success!`)
      } catch (execError) {
        console.warn(`   ⚠️  Subtitle download error:`, execError.message)
